...
def check_or_import_cities_from_file_to_db(path_file: str, model: City = City):
    """
    ...
    В случае изменении списка в csv, при перезапуске актуализирует
    список городов.
    """
    ...
    for row in DictReader(open(path_file)):
...
def get_list_cities(model: City = City):
    """Получить список городов из БД."""
    return [*model.select().execute()]


def get_weather_call(cities_list: list, model: WeatherCall = WeatherCall):
    """
